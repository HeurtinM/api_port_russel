...
document.getElementById('reservationForm').addEventListener('submit', async (event) => {
    ...
    let result = await fetch("https://api-port-russel-je588cze7-heurtinms-projects.vercel.app/catways/" + catwayID + "/reservations/" + ID);
...
document.getElementById('listReservations').addEventListener('click', async () => { 
    try {
        let catwaysResult = await fetch('https://api-port-russel-je588cze7-heurtinms-projects.vercel.app/catways/');
        let catways = await catwaysResult.json();

        let result = await fetch(`https://api-port-russel-je588cze7-heurtinms-projects.vercel.app/catways/${catways[0].catwayNumber}/reservations/`); //utilise le numero du premier catway enregistrer pour accedder a la route.De cette manière tant qu'il reste au moins un catway, la fonction fonctionnera toujours
...
document.getElementById('catwayNumberForReservation').addEventListener('change', function() {
        ...
        document.getElementById('addReservationForm').action = `https://api-port-russel-je588cze7-heurtinms-projects.vercel.app/catways/${catwayID}/reservations/`;
});
...
document.getElementById('updateReservationForm').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch(`https://api-port-russel-je588cze7-heurtinms-projects.vercel.app/catways/${reservationToUpdateCatway}/reservations/${reservationToUpdate}`, {
            method: 'PUT',
            headers: {
...
document.getElementById('deleteReservationForm').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch(`https://api-port-russel-je588cze7-heurtinms-projects.vercel.app/catways/${reservationToDeleteCatway}/reservations/${reservationToDelete}`, {
            method: 'DELETE',
        });
...
async function listIDForDropDown(catwayID, dropdownID) {
    try {
        let result = await fetch(`https://api-port-russel-je588cze7-heurtinms-projects.vercel.app/catways/${catwayID}/reservations/`);
        let reservations = await result.json();
