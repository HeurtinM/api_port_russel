...
document.getElementById('listReservations').addEventListener('click', async () => { 
    try {
        ...
        let result = await fetch(`http://localhost:3000/catways/${catways[0].catwayNumber}/reservations/`); //utilise le numero du premier catway enregistrer pour accedder a la route.De cette manière tant qu'il reste au moins un catway, la fonction fonctionnera toujours
...
document.getElementById('updateReservationForm').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch(`http://localhost:3000/catways/${reservationToUpdateCatway}/reservations/${reservationToUpdate}`, {
            method: 'PUT',
            headers: {
...
document.getElementById('deleteReservationForm').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch(`http://localhost:3000/catways/${reservationToDeleteCatway}/reservations/${reservationToDelete}`, {
            method: 'DELETE',
        });
